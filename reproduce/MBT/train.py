...
from dataloader import AV_Dataset
from mbt import AVmodel
...
def parse_options():
    ...
    parser.add_argument('--num_classes', type=int, default=101, help='number of latent tokens')
...
def train_one_epoch(train_data_loader,model,optimizer,loss_fn, device):
    ...
    for spec, imgs, labels in tqdm(train_data_loader):
...
def val_one_epoch(val_data_loader, model,loss_fn, device):
    ...
    with torch.no_grad():
      for spec, imgs, labels in tqdm(val_data_loader):
...
def train_test(args):
    ...
    train_data_list, train_labels = get_data()
    test_data_list, test_labels = get_data(mode='test')
    spec_mean = torch.load('spec_mean.pt')
    spec_std = torch.load('spec_std.pt')
    train_dataset = AV_Dataset(train_data_list, "/nas_data/WTY/dataset/UCF-101/UCF-WAV/", "/nas_data/WTY/dataset/UCF-101/UCF-Frame/", 
                       train_labels, spec_mean, spec_std)
    test_dataset = AV_Dataset(test_data_list, "/nas_data/WTY/dataset/UCF-101/UCF-WAV/", "/nas_data/WTY/dataset/UCF-101/UCF-Frame/", 
                       test_labels, spec_mean, spec_std)
    trainloader = DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=collate_fn, shuffle=True, num_workers=16, pin_memory=True)
    testloader = DataLoader(test_dataset, batch_size=args.batch_size, collate_fn=collate_fn, shuffle=False, num_workers=16, pin_memory=True)
